...
- Documentation synchronized with current code and repository layout:
  - `README.md` scoring description aligned to current implementation
    ...
    reference-area workflow).
  - `vignettes/simplestart.qmd` paths updated from obsolete
    `inst/examples/*` to existing `inst/ovc_demo/project/*` fixtures.
  - `vignettes/package-overview.qmd` clarified
    [`distances()`](https://rkrug.github.io/openalexVectorComp/reference/distances.md)
    as non-exported internal helper.
  - OpenAI batch vignette and other technical vignettes spot-checked for
    API naming consistency.
- Package version bumped in `DESCRIPTION`:
  - `Version: 0.1.2` -\> `Version: 0.1.3`
- Release commit scope includes current repository cleanup changes in
  this branch, including removal of stale helper scripts under:
  - `inst/qdrant functions/`
- Release-check caveats:
  - `R CMD check --no-manual --no-examples --no-tests .` may still emit
    package-structure notes/warnings if local non-package artifacts are
    present (for example temporary check directories).

## Scope

This note tracks all implementation/documentation changes made in this
workstream for `openalexVectorComp` (not only backend refactors).

## Change Log

### 1) Package/docs alignment after rename

- Updated references from `autotagr` to `openalexVectorComp` in key
  docs.
...
- Central dispatch now supports `provider = "hf"`, `"openai"`, or
  `"tei"`.
...
- Standardized provider token handling to one environment variable:
  - `OVC_API_TOKEN`
- Adapter sends this token as `Authorization: Bearer <token>` when
  present.
...
- Added generic embedding entry points:
  - [`embed_texts()`](https://rkrug.github.io/openalexVectorComp/reference/embed_texts.md)
    (single-call text embedding via configured backend)
  - [`embed_corpus()`](https://rkrug.github.io/openalexVectorComp/reference/embed_corpus.md)
    (streaming corpus embedding pipeline)
...
- Exports:
  ...
  - `prototype_centroid()` removed from public API; internal helper
    moved into `R/distance_reference_cosine.R` as
    `centroid_prototype()`.
- Documentation:
  - maintained via roxygen2 and regenerated `man/` + `NAMESPACE`.
  - added detailed Quarto vignette:
    - `vignettes/backend-architecture.qmd`
    - covers backend internals, dispatch order, sequence and flowchart
      diagrams, and how to add new providers.
  - added detailed TEI operations vignette:
    - `vignettes/tei-server-operations.qmd`
    - documents shell-based TEI process handling (start/stop/health),
      now that lifecycle helpers were removed from package code.
...
- TEI backend probing now uses internal lightweight HTTP `/info` probing
  in `R/embed_backend_tei.R`.
...
- Added HF integration helpers:
  ...
  - skips tests unless `OVC_API_TOKEN` is set and HF endpoint is
    reachable.
...
- Removed unconditional `skip_on_cran()` from HF helper so local
  integration tests can run without requiring `NOT_CRAN=true`.
- `4xx` payload/model mismatches no longer force an automatic skip,
  making test behavior easier to diagnose.
...
- Added offline unit/integration coverage in
  ...
  - [`fit_ridge()`](https://rkrug.github.io/openalexVectorComp/reference/fit_ridge.md)
    model persistence and class checks
  - [`distance_reference_cosine()`](https://rkrug.github.io/openalexVectorComp/reference/distance_reference_cosine.md)
    output validity and positive-vs-negative separation
  - [`distance_ridge()`](https://rkrug.github.io/openalexVectorComp/reference/distance_ridge.md)
    score range checks (`[0, 1]`) and separation checks
  - internal
    [`distances()`](https://rkrug.github.io/openalexVectorComp/reference/distances.md)
    helper join behavior on overlapping `id` keys
- Tests use a synthetic local Parquet embeddings dataset and do not
  require API tokens or network access.

### 13) Validation and faulty-input test expansion

- Added `tests/testthat/test-validation-inputs.R` with negative/edge
  cases to increase branch coverage in lower-covered modules:
  ...
  - backend dispatch validation:
    - invalid backend objects/providers in
  ...
  - internal backend helper behavior:
    ...
    - `.embedding_with_retry()` success-after-retries and terminal
      failure
...
- Updated metadata handling in:
  ...
  - [`distance_ridge()`](https://rkrug.github.io/openalexVectorComp/reference/distance_ridge.md)
    read/copy path
- Added backward compatibility in
  [`distance_ridge()`](https://rkrug.github.io/openalexVectorComp/reference/distance_ridge.md):
...
- Expanded metadata payload to better support reproducibility:
  ...
  - `backend.timeout`
  - `backend.retries`
- Because `embed_model.yaml` is a non-parquet sidecar in dataset
  directories, Arrow dataset openings now use:
  - `factory_options = list(exclude_invalid_files = TRUE)` in pipeline
    and plotting paths that scan these directories.
...
- [`embedding_backend_save()`](https://rkrug.github.io/openalexVectorComp/reference/embedding_backend_save.md)
  writes YAML in the same field shape as
  [`embedding_backend_config()`](https://rkrug.github.io/openalexVectorComp/reference/embedding_backend_config.md)
  output:
...
- [`embedding_backend_read()`](https://rkrug.github.io/openalexVectorComp/reference/embedding_backend_read.md)
  returns a normalized backend config by calling
  [`embedding_backend_config()`](https://rkrug.github.io/openalexVectorComp/reference/embedding_backend_config.md)
  under the hood.
- Backward compatibility:
  - [`embedding_backend_read()`](https://rkrug.github.io/openalexVectorComp/reference/embedding_backend_read.md)
    also accepts legacy nested metadata format (`model` + `backend`
    blocks) from earlier alpha revisions.
- [`embed_corpus()`](https://rkrug.github.io/openalexVectorComp/reference/embed_corpus.md)
  now writes metadata through
  [`embedding_backend_save()`](https://rkrug.github.io/openalexVectorComp/reference/embedding_backend_save.md).
- [`distance_ridge()`](https://rkrug.github.io/openalexVectorComp/reference/distance_ridge.md)
  now reads metadata through
  [`embedding_backend_read()`](https://rkrug.github.io/openalexVectorComp/reference/embedding_backend_read.md)
  when deriving model partition naming.

### 16) Coverage-focused offline test expansion

- Added `tests/testthat/test-embedding-offline.R` to increase branch
  coverage without network/API tokens:
  - [`embed_texts()`](https://rkrug.github.io/openalexVectorComp/reference/embed_texts.md)
    forwarding behavior
  - HF/OpenAI/TEI adapter batching paths
  - adapter malformed-payload error paths
  - [`embed_corpus()`](https://rkrug.github.io/openalexVectorComp/reference/embed_corpus.md)
    incremental behavior and `delete_existing` branch
- Expanded `tests/testthat/test-validation-inputs.R` with a successful
  [`calibrate_threshold()`](https://rkrug.github.io/openalexVectorComp/reference/calibrate_threshold.md)
  run using `labels_parquet`, `metric = "precision_at_recall"`, and
  explicit thresholds.
...
- [`embed_corpus()`](https://rkrug.github.io/openalexVectorComp/reference/embed_corpus.md)
  now validates preprocessor outputs:
  - must be a data frame
  - must include `id`, `text`, `text_hash`
  - no ids outside the current batch
  - no duplicated ids
  - drops empty/NA `id` or `text`
- Resume/skip logic remains `id + text_hash`, but hash is now produced
  by the preprocessor.
- Output parquet now preserves extra preprocessor-provided provenance
  columns.
...
- Covers:
  ...
  - step-by-step examples, including before/after kept vs discarded
    outputs
...
The complexity does not mainly come from TEI itself. It comes from
combining these concerns:
...
Supporting both local TEI and hosted inference APIs can increase
variability, but resume/reliability requirements are the biggest
complexity driver.
...
- one TEI embedding function with fixed arguments (`url`, `texts`,
  `batch_size`),
...
- `roxygen2::roxygenise(load = "source")` was used because full package
  loading in this environment can fail due to system restrictions around
  some dependencies.
- Existing unrelated repo changes were preserved.
- `inst/openalex_embedding_pipeline_v4.md` was intentionally left in
  place.

## Ongoing Updates

This file should be updated as additional changes are made in this
branch so it remains a single place to review implementation decisions
and rationale.

### 21) Label-partitioned embeddings and pairwise prototype distances

- [`embed_corpus()`](https://rkrug.github.io/openalexVectorComp/reference/embed_corpus.md)
  gained a new `label` argument (default `"corpus"`).

- Embeddings path changed from `model/batch` to `model/label/batch`:

  - `embeddings/model_id=<...>/label=<label>/batch=<n>/embeddings-*.parquet`

- `delete_existing = TRUE` now deletes only the selected `label`
  subtree.
...
- [`distance_reference_cosine()`](https://rkrug.github.io/openalexVectorComp/reference/distance_reference_cosine.md)
  was redesigned from `included/excluded` centroid margin to label-based
  cosine distance computation:

  - inputs: `corpus_label`, `reference_label`
  - computes pairwise matrix between all `reference` and `corpus`
    vectors
  - output path:
...
- [`fit_ridge()`](https://rkrug.github.io/openalexVectorComp/reference/fit_ridge.md)
  was redesigned to fit a reference-area model instead of binary ridge
...
- Metadata copy for distance outputs now includes ridge-mode context
  fields:
...
- [`calibrate_threshold()`](https://rkrug.github.io/openalexVectorComp/reference/calibrate_threshold.md)
  was fixed to only read `included`/`excluded` CSVs when
  `labels_parquet` is not provided.
...
- [`distance_reference_cosine()`](https://rkrug.github.io/openalexVectorComp/reference/distance_reference_cosine.md)
  was further simplified to be distance-only and single-file:
...
- Output schema now encodes both pairwise and centroid distances in one
  wide matrix:
  - first column: `id` (corpus ids + one `"centroid"` row for corpus
    centroid),
  - remaining columns: reference ids + one `"centroid"` column for
    reference centroid.
- Cell semantics:
...
- Added exported
  ...
  to transform the full output matrix from
  [`distance_reference_cosine()`](https://rkrug.github.io/openalexVectorComp/reference/distance_reference_cosine.md).
- Supports scoring methods:
  - `linear` (default): `score = 1 - distance`
  - `exponential`: `score = exp(-alpha * distance)`
- Preserves matrix shape (`id` + all value columns) and writes scored
  output under `score_reference_cosine/...`.
...
- Added install-time demo fixtures in `inst/ovc_demo/`:
  - `project/corpus/corpus_small.parquet` (\<= 100 rows,
    `id/title/abstract`)
  - `project/reference_corpus/reference_small.parquet` (\<= 10 rows,
    `id/title/abstract`)
- Added Quarto analysis template in
  `inst/ovc_demo/openalex_demo_analysis.qmd` covering:
  ...
  - [`distance_ridge()`](https://rkrug.github.io/openalexVectorComp/reference/distance_ridge.md)
  - [`score_ridge()`](https://rkrug.github.io/openalexVectorComp/reference/score_ridge.md)
- Added tests in `tests/testthat/test-demo-runner.R` for setup,
  overwrite handling, fixture/schema caps, template call coverage, and
  optional render.
...
- Submission now performs preflight planning before remote API calls:
  - hard caps enforced: `max_requests_per_job <= 50000`,
    `max_job_bytes <= 200MB`
  - conservative defaults: `max_requests_per_job = 20000`,
    `max_job_bytes = 150MB`
...
- Collected outputs are written to the canonical embeddings layout:
  - `embeddings/model_id=<...>/label=<...>/batch=<n>/embeddings-*.parquet`
    ensuring downstream compatibility with distance/score/plotting
    flows.
