...
- `project_dir/embeddings/model_id=<model>/batch=*/embeddings-*.parquet`
- metadata in:
...
flowchart TB
  ...
  B --> D[model_id=.../batch=*/embeddings-*.parquet]
  B --> E[dry_run_cleaning_model_id=...parquet]
```
...
1. **Prototype distance**
   - interpretable geometric signal from included/excluded centroids.

2. **Ridge logistic score**
   - supervised linear decision in embedding space.
